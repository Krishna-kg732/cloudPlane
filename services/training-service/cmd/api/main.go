...
import (
	...
	// "cloudplane/training-service/internal/server"
)
...
func main() {
	...
	// svc := server.NewTrainingServer()
	// pb.RegisterTrainingServiceServer(grpcServer, svc)
	...
	go func() {
		log.Printf("Starting training-service gRPC server on port %s", cfg.Port)
